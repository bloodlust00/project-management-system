...
async def test_manager_can_create_project(client: AsyncClient, db_session):
    ...
    manager = await create_user_with_role(db_session, "manager@example.com", "Manager")
    
    # Login to obtain token
    login_res = await client.post("/api/v1/auth/login", json={
        "email": "manager@example.com",
        "password": "Password123!" # matches pre-calculated hash mock
    })
    ...
    proj_payload = {
        ...
        "description": "Created by test manager."
    }
...
async def test_employee_blocked_from_deleting_projects(client: AsyncClient, db_session):
    ...
    employee = await create_user_with_role(db_session, "employee@example.com", "Employee")
    manager = await create_user_with_role(db_session, "project_owner@example.com", "Manager")
    ...
    login_emp = await client.post("/api/v1/auth/login", json={
