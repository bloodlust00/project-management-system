...
class UserRepository(BaseRepository[User]):
    ...
    async def get_with_roles(self, user_id: Any) -> Optional[User]:
        ...
        query = select(User).filter(User.id == user_id, User.is_deleted == False).options(selectinload(User.roles))
        result = await self.db.execute(query)
        return result.scalars().first()
    ...
    async def get_by_ids(self, ids: List[Any]) -> List[User]:
        ...
        query = select(User).filter(User.id.in_(ids), User.is_deleted == False)
        result = await self.db.execute(query)
        return list(result.scalars().all())
