...
async def get_me(current_user: User = Depends(get_current_user)):
    ...
    return APIResponse[UserResponse](
        success=True, message="Profile details fetched successfully", data=UserResponse.model_validate(current_user)
    )
...
async def update_me(
    ...
    updated_user = await user_service.update_user(
        user_id=current_user.id, user_in=user_in, current_user_id=current_user.id
    )
    return APIResponse[UserResponse](
        success=True, message="Profile details updated successfully", data=UserResponse.model_validate(updated_user)
    )


@router.get(
    "/", response_model=APIResponse[PaginatedData[UserResponse]], dependencies=[Depends(RoleChecker(["Admin"]))]
)
    ...
    return APIResponse(
        ...
        data=PaginatedData(
            ...
            pagination=PaginationMeta(
                total=total, page=(skip // limit) + 1 if limit > 0 else 1, size=limit, pages=pages
            ),
        ),
    )
...
async def get_user_by_id(
    ...
    return APIResponse[UserResponse](
        success=True, message="User profile resolved successfully", data=UserResponse.model_validate(user)
    )


@router.put(
    "/{user_id}/roles", response_model=APIResponse[UserResponse], dependencies=[Depends(RoleChecker(["Admin"]))]
)
async def update_user_roles(
    ...
    updated_user = await user_service.update_user_roles(
        user_id=user_id, role_names=roles_in.role_names, current_user_id=current_user.id
    )
    return APIResponse[UserResponse](
        success=True, message="User roles updated successfully", data=UserResponse.model_validate(updated_user)
    )
...
async def delete_user(
    ...
    return APIResponse(success=True, message="User account deleted successfully.")
