...
async def get_task_comments(
    task_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    ...
    return APIResponse[List[CommentResponse]](success=True, message="Comments fetched successfully", data=serialized)
...
async def create_comment(
    ...
    return APIResponse[CommentResponse](
        success=True, message="Comment added successfully", data=CommentResponse.model_validate(comment)
    )


@router.delete(
    "/{comment_id}", response_model=APIResponse, dependencies=[Depends(PermissionChecker(["comment:delete"]))]
)
async def delete_comment(
    comment_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
