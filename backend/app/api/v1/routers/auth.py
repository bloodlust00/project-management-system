...
async def login(user_login: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    return APIResponse[Token](success=True, message="Login successful", data=Token(**token_data))
...
async def refresh(refresh_in: TokenRefresh, db: AsyncSession = Depends(get_db)):
    ...
    return APIResponse[Token](success=True, message="Token rotated successfully", data=Token(**rotated_data))


@router.post("/logout", response_model=APIResponse)
async def logout(refresh_in: TokenRefresh, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    await auth_service.logout(refresh_token=refresh_in.refresh_token, access_token=access_token)
    return APIResponse(success=True, message="User logged out successfully. Tokens blacklisted.")
