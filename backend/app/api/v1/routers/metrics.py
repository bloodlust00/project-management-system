...
async def metrics_check(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        proj_count = await db.execute(select(func.count(Project.id)).filter(Project.is_deleted == False))
        task_count = await db.execute(select(func.count(Task.id)).filter(Task.is_deleted == False))
        user_count = await db.execute(select(func.count(User.id)).filter(User.is_deleted == False))
...
async def dashboard_stats(
    ...
    # Projects count
    proj_query = select(func.count(Project.id)).filter(Project.is_deleted == False)
    ...
    for task_status in TaskStatus:
        t_query = select(func.count(Task.id)).filter(Task.status == task_status, Task.is_deleted == False)
    ...
    for prio in TaskPriority:
        p_query = select(func.count(Task.id)).filter(Task.priority == prio, Task.is_deleted == False)
        p_result = await db.execute(p_query)
        priority_stats[prio.value] = p_result.scalar() or 0

    # Comments count
    com_query = select(func.count(Comment.id)).filter(Comment.is_deleted == False)
...
@router.get("/audit-logs", response_model=APIResponse[PaginatedData[ActivityLogResponse]], dependencies=[Depends(PermissionChecker(["audit:read"]))])
